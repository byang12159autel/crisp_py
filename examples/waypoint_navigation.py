...
nodes = {
    ...
    "ReadyInsert":
        Pose(
        position=np.array([ 0.41032407 ,-0.01228578 , 0.276]),
        orientation=Rotation.from_euler('xyz', [-179.86306532 ,  1.0  , -2.99437881], degrees=True)
        ),
    "FullInsert":
        Pose(
        position=np.array([0.45275626 ,-0.01228578 ,0.28]),
        orientation=Rotation.from_euler('xyz',[-179.86306532 ,  1.0  , -2.99437881], degrees=True)
        ),
    "Pause": {
        "sleep": 10
